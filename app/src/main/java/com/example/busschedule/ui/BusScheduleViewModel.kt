...
class BusScheduleViewModel(private val repository: BusScheduleRepository): ViewModel() {
    ...
    fun getFullSchedule(): Flow<List<BusSchedule>> = repository.getFullSchedule()

    // Get example bus schedule by stop
    fun getScheduleFor(stopName: String): Flow<List<BusSchedule>> = repository.getScheduleFor(stopName)
    companion object {
        val factory : ViewModelProvider.Factory = viewModelFactory {
            initializer {
                ...
                val repository = BusScheduleRepository(database.busScheduleDao())
